...
describe("GradesEvaluationsPage — Saisie des notes brouillon/ouverte/validée", () => {
  ...
  it("Enseignant : handleSaveGrades sans evaluation.teacherId n'envoie pas teacherId", async () => {
    ...
    expect(upsertNoteApi).toHaveBeenCalledTimes(1);
    const [payload] = upsertNoteApi.mock.calls[0];
    expect(payload.teacherId).toBeUndefined();
    expect(payload.authorId).toBeUndefined();
  ...
  it("Préfet : handleSaveGrades refuse une évaluation sans teacherId pédagogique", async () => {
    sessionUser.current = {
      id: "prefet-fideline",
      role: "Préfet des études",
      schoolCode: "SCH-001",
...
describe("GradesEvaluationsPage — refresh puis mutation Préfet", () => {
  ...
  it("Préfet handleSaveGrades : teacherId pédagogique réinjecté, pas authorId", async () => {
    const evaluation = { ...evaluationBase, status: "Validée" };
    evaluationsForPage.current = [evaluation];
    dataState.current = { ...dataState.current, evaluations: [evaluation], notes: [refreshedNote] };
    renderPage();
