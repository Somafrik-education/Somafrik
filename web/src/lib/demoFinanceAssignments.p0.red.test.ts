/**
 * DEMO-FINANCE-ASSIGNMENTS-P0 — tests RED (audit, pas de GREEN).
 *
 * Triplet Démo constaté après #678 :
 ...
 *
 * Ces tests ancrent le contrat métier observé en recette. Ils doivent rester
 * rouges tant que la cause racine n'est pas corrigée. Aucun fallback
...
/** Projection GET /api/payments (mapPaymentRow) : pas de schoolId. */
...
describe("DEMO-FINANCE-ASSIGNMENTS-P0 RED", () => {
  it("DEMO-FIN-RED-01 — session CD-IN-26-001 voit les obligations canoniques de son schoolId", () => {
    ...
    const presented = presentActiveSchoolState(hydrated, user.schoolCode);
  ...
  it("DEMO-FIN-RED-02 — session Démo voit les paiements canoniques de son schoolId", () => {
    ...
    expect(payments.every((row) => !("schoolId" in row) || row.schoolId == null)).toBe(true);
  ...
  it("DEMO-PRES-RED-01 — classe 1ère A avec teacher_assignment canonique n'est pas projetée sans enseignant", () => {
    const classRow = {
      id: CLASS_1ERE_A_ID,
  ...
  it("DEMO-TENANT-RED-01 — schoolId, schoolCode et loginCode ne sont jamais des identifiants équivalents", () => {
    ...
    const presentedByInternalCode = presentActiveSchoolState(hydrated, DEMO_SCHOOL_CODE);
    const presentedByLoginCode = presentActiveSchoolState(hydrated, DEMO_LOGIN_CODE);
