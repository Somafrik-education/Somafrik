...
export function GradeEntryGrid({
  ...
  return (
    <div>
      {!canEdit && (evaluation.status === "Publiée" || evaluation.status === "Annulée") ? (
        <div className="mb-4 rounded-md border border-amber-200 bg-amber-50 px-3 py-2" role="status">
          <p className="text-sm font-semibold text-amber-900">Saisie fermée</p>
          <p className="text-sm text-amber-800">
            Cette évaluation est {evaluation.status.toLowerCase()} : plus de saisie possible.
          </p>
        </div>
      ) : null}
