              ...
              <article key={item.id} className="grid items-center gap-8 md:grid-cols-2 md:gap-12">
                ...
                <figure className={`min-w-0 ${reverse ? "md:order-1" : ""}`}>
                  <div className="mx-auto w-[11.5rem] sm:w-[12.5rem] md:mx-0 md:w-[13rem]">
                    {/* Cadre CSS discret autour de la capture originale — pas un faux téléphone. */}
                    <div className="overflow-hidden rounded-xl bg-white shadow-[0_10px_24px_-20px_rgba(15,23,42,0.35)] ring-1 ring-slate-200">
                      <img
                        src={item.src}
                        alt={item.alt}
                        width={item.width}
