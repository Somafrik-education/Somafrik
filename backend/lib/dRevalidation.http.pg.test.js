...
/**
 * Lot D finale — extras PR-audit / PR-scope + SY-02 / SY-09 / SY-10
 * après #435 / #436 / #437 / #439.
...
async function main() {
  ...
  try {
    ...
    check(
      "PR-scope-teacher-assigned",
      getTeacherAssigned.status === 200 &&
        teacherAssignedRows.some(isPresenceA) &&
        !teacherAssignedRows.some(isPresenceB),
      `status=${getTeacherAssigned.status} count=${teacherAssignedRows.length}`,
    );
    ...
    check(
      "PR-scope-admin-A",
      getAdminA.status === 200 && adminARows.some(isPresenceA) && !adminARows.some(isPresenceB),
      `status=${getAdminA.status} count=${adminARows.length}`,
    );
