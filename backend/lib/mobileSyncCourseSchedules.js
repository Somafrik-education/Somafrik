...
/**
 * Snapshot L1 CourseSchedules : weekly slots PostgreSQL, curseur opaque, scopeHash.
 * Teacher : UUID live ET paires d'affectations actives — pas w.teacher_id seul.
 *
 * @param {{
 *   principal: object,
 *   cursor?: unknown,
 ...
 * }} args
 */
async function handleMobileSyncL1CourseSchedules(args) {
  ...
  if (typeof repository.listCourseSchedulesForMobileSync !== "function") {
  ...
  let input;
  try {
    ({ scopeHash, scope, input } = await resolveLiveCourseSchedulesSyncSnapshot(
      repository,
      principal,
      schoolRef,
    ));
  ...
  if (scope.scopeKind !== "none" && !liveSnapshotHasCourseSchedulesRead(input)) {
    const result = protocolErrorBody(
      403,
      PERMISSION_DENIED,
  ...
  const rows = await repository.listCourseSchedulesForMobileSync(schoolCode, queryOptions);
...
module.exports = {
  handleMobileSyncL1CourseSchedules,
  clampLimit,
  toIsoTimestamp,
};
