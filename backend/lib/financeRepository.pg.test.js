  ...
  try {
    ...
    const schoolA = await pool.query(
      `INSERT INTO schools (country_id, school_code, login_code, name, status)
       VALUES ($1, $2, 'CD-FM-26-001', 'Lycée A', 'active') RETURNING id, school_code, login_code`,
      [country.rows[0].id, leftoverA],
    );
    const schoolB = await pool.query(
      `INSERT INTO schools (country_id, school_code, login_code, name, status)
       VALUES ($1, $2, 'BI-FM-26-001', 'Lycée B', 'active') RETURNING id, school_code, login_code`,
        ...
        store.createSchoolPayment(
          {
            ...
            items: [{ feeType: "Non imputé", feeLabel: "Probe leftover", amount: 1 }],
            paymentMethod: "cash",
            paidAt: "2026-08-19",
            totalAmount: 1,
          },
          { ...admin, schoolCode: leftoverA },
        ),
      (error) =>
    ...
    assert.equal(byLogin.loginCode, schoolALogin);
