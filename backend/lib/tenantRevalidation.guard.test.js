...
test("revalidation: dual-identity A/B est exigée dans les preuves HTTP canoniques", () => {
  ...
  for (const src of [finance, ay, users]) {
...
test("revalidation: Enrollment HTTP utilise encore leftover JWT (FINDING documenté)", () => {
  const server = read("server.js");
  ...
  assert.match(getStudents, /req\.principal\?\.schoolCode/);
  assert.match(postEnroll, /req\.principal\?\.schoolCode/);
  assert.match(getById, /req\.principal\?\.schoolCode/);
