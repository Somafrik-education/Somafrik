...
assert.match(generalBoot, /somafrik_student_identity_taken/);
assert.match(generalBoot, /pg_attribute/);
assert.match(generalBoot, /pg_constraint/);
assert.match(generalBoot, /ACCESS EXCLUSIVE/);
assert.match(generalBoot, /MAX\(last_value\)/);
assert.match(generalBoot, /ON CONFLICT \(school_id\)/);
assert.doesNotMatch(generalBoot, /DROP TABLE(?: IF EXISTS)? student_general_code_counters(?:\s|;|$)/i);
assert.doesNotMatch(generalBoot, /ON CONFLICT DO NOTHING/);
