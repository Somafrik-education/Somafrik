...
test("RED-COM-01 P1 — adaptateur PUSH/EMAIL après persist C4", () => {
  ...
  assert.match(
    worker,
    /communicationsDispatcher|dispatchProcessedEvents/,
    "le worker C4 doit déclencher le dispatcher après drainOutbox",
  );
  assert.doesNotMatch(
    worker,
    /fanOutNotificationChannels/,
    "le worker ne doit pas appeler fanOutNotificationChannels en parallèle du dispatcher",
  );
  ...
  assert.ok(drainCall >= 0 && dispatchCall > drainCall, "dispatcher après drainOutbox, pas avant persist");
});
