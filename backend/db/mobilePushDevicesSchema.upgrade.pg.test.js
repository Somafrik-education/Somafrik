  ...
  try {
    ...
    await pool.query(
      `INSERT INTO mobile_push_devices (user_id, expo_push_token, platform, release_profile)
       VALUES ($1, 'ExponentPushToken[legacy-preview]', 'android', 'preview')`,
      [userId],
    );

    await pool.query(MOBILE_PUSH_DEVICES_SCHEMA_SQL);

    const columns = await pool.query(
      `SELECT column_name FROM information_schema.columns
       WHERE table_schema = 'public' AND table_name = 'mobile_push_devices'
    ...
    const row = await pool.query(
      `SELECT backend_environment, app_profile FROM mobile_push_devices WHERE expo_push_token = $1`,
      ["ExponentPushToken[legacy-preview]"],
    );
    assert.equal(row.rows[0].backend_environment, "preproduction");
    assert.equal(row.rows[0].app_profile, "preview");
    console.log("mobilePushDevicesSchema.upgrade.pg.test.js GO");
  } finally {
    await pool.end();
  }
}
