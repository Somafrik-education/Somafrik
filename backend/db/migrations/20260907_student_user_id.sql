...
-- (23514) même NOT VALID. On n'essaie pas de dupliquer l'expression de la CHECK :
-- chaque ligne est appliquée sous EXCEPTION check_violation (contrainte réelle).
-- Aucun DROP/VALIDATE, aucune réécriture d'identité.
...
BEGIN
  FOR rec IN
    SELECT st.id AS student_id, u.id AS matched_user_id
    FROM students st
  ...
  LOOP
    BEGIN
      UPDATE students
      SET user_id = rec.matched_user_id
      WHERE id = rec.student_id AND user_id IS NULL;
    EXCEPTION
      WHEN check_violation THEN
        RAISE NOTICE '20260907: student % exclu du backfill user_id (CHECK)', rec.student_id;
    END;
  END LOOP;
END $$;
...
BEGIN
  ...
  LOOP
    ...
    EXCEPTION
      WHEN check_violation THEN
        RAISE NOTICE '20260907: student % exclu du backfill user_id (CHECK civile)', rec.student_id;
    END;
  END LOOP;
END $$;
