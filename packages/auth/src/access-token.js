...
function cloneValidatedAccessToken(accessToken, status, revokedAt) {
  return createAuthSessionAccessToken({
...
function revokeAuthSessionAccessTokenUnchecked(accessToken, revokedAt) {
  let validated;
  try {
    validated = createAuthSessionAccessToken(accessToken);
  } catch {
    throw new AuthSessionAccessTokenValidationError("auth session access token is invalid");
  }
  ...
  if (validated.status === AUTH_SESSION_ACCESS_TOKEN_STATUS.REVOKED) {
    if (validated.revokedAt === canonicalRevokedAt) {
      return cloneValidatedAccessToken(
        validated,
        AUTH_SESSION_ACCESS_TOKEN_STATUS.REVOKED,
        validated.revokedAt,
      );
    }
    throw new AuthSessionAccessTokenValidationError("access token already revoked");
  }
...
export function revokeAuthSessionAccessToken(accessToken, revokedAt) {
  try {
    return revokeAuthSessionAccessTokenUnchecked(accessToken, revokedAt);
  } catch (error) {
    if (
      error &&
      error.name === "AuthSessionAccessTokenValidationError" &&
