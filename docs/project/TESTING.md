...
| `npm run verify:data-export-safety` | LOT 6 export : 401/403, tenant JWT, enveloppe `somafrik-export` v1, pas de secrets, `includedDomains` exact, audit `export_school_data` sans payload, PUT state 410, aucun bouton Restaurer / input file / `partial: false` |
