...
| `npm run verify:documents-exams-data` | LOT 5 examens/bulletins/documents : CRUD `exams` / `report_cards` / `report_card_templates` / `school_documents`, PUT residual interdit (`LEGACY_*_WRITE_FORBIDDEN`), tenant JWT, UUID étranger 404, 409, audit rollback, residual exact → strip, residual ambigu → STOP (`documentsExams.pg.test.js` si `DATABASE_URL`) |
