...
Une fois `PUBLISHED`, le payload est **deep-freeze**. Le renderer LOT 5 / PDF / `/verify` lit **uniquement** `payloadForRender(sealed, signingKeyRing)`, qui : (1) refuse si `SHA-256(canonical_bytes) !== snapshot_sha256` (`SNAPSHOT_INTEGRITY`) ; (2) résout la clé publique par `signing_key_id` historique (`SNAPSHOT_SIGNING_KEY_UNKNOWN` si absente) ; (3) exige `Ed25519.Verify(canonical_bytes, snapshot_signature)` (`SNAPSHOT_SIGNATURE_INVALID`). Un hash seul ne suffit pas : réécrire payload + hash en base sans la clé privée échoue. Gates : `snapshot-immutability`, `snapshot-canonical-bytes-tamper-fails-closed`, `snapshot-canonical-jcs`, `snapshot-signature-required-before-render`.
