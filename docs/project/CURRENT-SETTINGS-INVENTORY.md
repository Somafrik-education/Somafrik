...
| Code établissement | lecture seule sur le profil (`login_code`) | PostgreSQL trigger `somafrik_prepare_school_login_code` — **pas** `generateSchoolCode` client | Canonique PG `schools.login_code` | `schools.login_code` UNIQUE ; `schools.school_code` alias interne | Création : `Établissements:CREATE` | Pays + année civile | Format V2 `CC-IN-26-001` ; `CD-2026-0001` legacy interdit à la création | `GET /api/schools` public | Tenant scope partout | Ancien format `CC-AAAA-nnnn` retiré de l'UI | P0 |
