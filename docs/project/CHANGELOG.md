...
- **LOT 11 P0-C / P1-D (revalidation CTO)** : pin mapping artefact↔bundle (`mapped id/version/sha256`) invalidé au replace ; `READY_FOR_REVIEW` refuse un current non mappé ; statut relus sous verrou request ; preview Web établissement+Superadmin inline (`nosniff`, `private, no-store`). Ticket #655 / PR #657.
