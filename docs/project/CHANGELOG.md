...
- **RC1 G1 — requalification après #733 (#719 / #722 / #741)** : rebase `develop@109fa474` ; gates 21/24 ; extra PG/tenant/#732/#503 PASS ; **P0/P1 reproduits = 0**. Perf mémoire PASS (pas une preuve PG). HOLD faute de preuves UI/AAB/perf préprod. Production unmodified. HOLD READY+MERGE.
- **RC1 G1 — qualification HOLD (#719 / #720)** : rapports `docs/release/RC1-*.md` ; harness `verify:rc1-gates` / `verify:rc1-performance` ; requalification #717/#645/#646/#499/#503/#510. Production unmodified. HOLD READY+MERGE.
...
- **LOT 11 — Artefact source établissement + mapping traçable** : upload établissement PDF/JPEG/PNG (magic bytes, 10 MiB, SHA-256, clé opaque `YYYY/<uuid>`) vers `SOMAFRIK_REPORT_CARD_SOURCE_STORAGE` ; mapping Superadmin explicite vers `AcademicRuleProfile + ReportCardSchema + RenderingTemplate` ; preview Web ; aucun OCR/IA, aucun Mobile. Gate `verify:report-card-lot11`. HOLD READY+MERGE. Ticket #655.
- **LOT 11 P0-E (revalidation CTO)** : `markReadyForReview` exige que le pin source (`profile|schema|template` id/version/`spec_sha256`) soit identique au bundle courant de la demande, en plus de l’artefact id/version/hash. Tests PG : panne après `bindBundle` avant `saveMapping` ; rebind X→Y même artefact. Ticket #655 / PR #657.
- **LOT 11 — Artefact source établissement + mapping traçable (RED)** : tests nommés upload/sécurité/versioning/workflow Web ; inventaire stockage canonique (réutilisation du disque durable Communications, namespace dédié, pas de nouveau provider) ; gate `verify:report-card-lot11`. Production code unmodified. HOLD READY+MERGE. LOT 12+ interdit. Ticket #655.
...
- **Demandes d'essai 1 mois (Web + API, hors Mobile)** : CTA vitrine/`/connexion` vers `/demande-essai` ; formulaire public avec consentement obligatoire (aucun tenant/utilisateur/abonnement créé) ; table `trial_access_requests` + `POST /api/public/trial-requests` (rate limit, anti-doublon 409) ; inbox Superadmin `/abonnements/demandes-essai` + `GET /api/backoffice/trial-requests` ; activation **Standard + 30 jours d'essai** (`OFFER-STANDARD`, jamais `OFFER-TRIAL`) ; invariant PG/backend `schools.trial_used` (un seul essai par établissement) ; liste pays Afrique francophone (source Web `francophoneAfricaCountries.ts`) ; après persistance, notification e-mail SMTP optionnelle vers `contact@somafrik.app` (échec mailer = log, le lead reste) ; politique de confidentialité : finalité demandes d'essai, rétention 24 mois, sous-traitance SMTP documentée.
...
- **Comptes utilisateurs V2 — identité canonique multi-rôles** : table `user_roles` (GRANT/REVOKE transactionnels, index unique des rôles actifs) ; création d’identité sans rôle (`Sans affectation`) ; UUID et `user_code` générés backend ; UI **Modifier | Attribuer** ; `PARENT`/`STUDENT` non attribuables ; enseignant-parent = un seul `users` ; `POST /api/teachers` 403 (`TEACHER_IDENTITY_MUST_COME_FROM_USERS`) ; `verify:user-role-lifecycle`.
- **LOT 6 — Export versionné + lockout PostgreSQL** : `GET /api/data-export` enveloppe `somafrik-export` v1 (domaines réellement lus : `schoolSettings`, `students`, `classes`, `teachers`, `audit` Superadmin) ; **snapshot PostgreSQL `READ ONLY` + `REPEATABLE READ`** (pas de mélange d’instants si une inscription/classe/enseignant change pendant l’export) ; aucun secret ; tenant JWT (Admin School ignore le `schoolCode` query) ; audit `export_school_data` après le snapshot, sans payload, fail-closed. Restauration globale Web **retirée** ; `DataContext.update({ partial: false })` refusé. Lockout : table `login_lockouts` SoT (clé `school_scope` + `identifier_normalized`, `INSERT … ON CONFLICT` atomique, 5 échecs / 15 min) ; plus de `Map` processus en runtime PG ; `POST /api/backoffice/e2e/clear-login-lockout` = 404 hors `SOMAFRIK_E2E` non-prod ; `SOMAFRIK_DISABLE_LOGIN_LOCKOUT` interdit en production.
