...
- **LOT 11 P0-A / P1-B (revalidation CTO)** : remplacement d’artefact source en transaction PostgreSQL avec verrou `(school_id, request_id)` (`FOR UPDATE` + advisory lock), version sous verrou, archivage+INSERT+audit atomiques, compensation blob si le commit échoue ; audit `MAP_EXPLICIT` persiste artefact version/hash et bundle id/version/hash (jointure testée). Tests PG concurrents et d’échec entre archive et création. Ticket #655 / PR #657.
