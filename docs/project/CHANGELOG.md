...
- **DEMO-0 — Contrat vitrine → Démo** : architecture figée `somafrik.app` → `/demo` → session opaque → `demo.somafrik.app` / `api-demo.somafrik.app` / PostgreSQL Demo ; `/demande-essai` reste distinct ; tests RED d’absence (aucune route, aucun env, aucun CTA) ; isolation CORS production, seed legacy et JWT-en-URL. Gate `verify:demo-lot0`. Aucune UI / infra Demo dans ce lot. Audit `docs/audits/demo-vitrine-cto-2026-09-14.md`.
