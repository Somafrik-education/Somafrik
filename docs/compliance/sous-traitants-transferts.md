...
| **SMTP transactionnel (opérateur)** | Notification e-mail des demandes d’essai vers `contact@somafrik.app` via `communication_channel_deliveries` + drain SMTP existant | Champs prospect uniquement (nom, rôle, établissement, pays, ville, téléphone, e-mail, effectif, référence publique) — **pas** le dossier scolaire ; payload delivery sans secret SMTP | Région du prestataire SMTP choisi par l’opérateur (transport générique `nodemailer` du drain C4 ; ex. Resend, Amazon SES, SendGrid SMTP, Brevo-as-SMTP) | DPA / SCC du prestataire retenu ; pas de secret SMTP dans git | logs et rétention du prestataire | `backend/lib/trialAccessRequestNotification.js` (enqueue) ; `backend/lib/communicationChannelFanout.js` `dispatchEmail` ; variables `SMTP_HOST`, `SMTP_PORT`, `SMTP_SECURE`, `SMTP_USER`, `SMTP_PASSWORD`, `MAIL_FROM`, `TRIAL_REQUEST_NOTIFY_TO` |
