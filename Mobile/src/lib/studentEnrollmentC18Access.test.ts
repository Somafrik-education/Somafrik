...
test("C18 Mobile RBAC : admin autorisé, parent/élève/teacher lecture refusés", () => {
  ...
  assert.equal(canMutateC18Mobile({ role: "parent_student", permissions: ["ALL_PRIVILEGES"] }), false);
  assert.equal(canMutateC18Mobile({ user: { role: "parent_student", permissions: ["Élèves:UPDATE"] } }), false);
