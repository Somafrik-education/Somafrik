...
const reportsScreen = read("screens/ReportCardsScreen.tsx");
assert.match(reportsScreen, /ExpandableEntityCard/, "PD-06 : liste bulletins = ExpandableEntityCard");
assert.match(reportsScreen, /expandedReportCardId/);
assert.match(reportsScreen, /ScrollView/);
assert.match(reportsScreen, /rows\.map\(\(card\) =>/);
assert.match(reportsScreen, /testID=\{DATA_TRUTH_TEST_IDS\.bulletinsList\}/);
assert.doesNotMatch(reportsScreen, /extraData=\{expandedReportCardId\}/, "PD-06 : pas de FlatList extraData");
assert.doesNotMatch(reportsScreen, /<FlatList/, "PD-06 : conserver ScrollView + map");
assert.match(reportsScreen, /nextExclusiveExpandedKey\(current, card\.id\)/);
assert.doesNotMatch(reportsScreen, /defaultExpanded=\{true\}/);
assert.match(reportsScreen, /downloadReportCardPdf/, "PD-06 : logique PDF inchangée");
assert.match(reportsScreen, /Linking\.openURL\(localUri\)/);
assert.match(reportsScreen, /filterRowsByStudentScope/, "PD-06 : student scope inchangé");
assert.match(reportsScreen, /resolveMobileStudentScope/);
const reportCard = sliceFirstCard(reportsScreen, "ExpandableEntityCard");
assert.ok(reportCard, "PD-06 : carte bulletin absente");
const reportOpening = cardOpening(reportCard);
assert.match(reportOpening, /title=\{/);
assert.match(reportOpening, /card\.studentName/);
assert.match(reportOpening, /subtitle=\{/);
assert.match(reportOpening, /period/);
assert.match(reportOpening, /badge=\{/);
assert.match(reportOpening, /card\.status/);
assert.doesNotMatch(reportOpening, /Visionner le bulletin/, "PD-06 : PDF hors résumé");
assert.doesNotMatch(reportOpening, /averageLabel/, "PD-06 : moyenne hors résumé");
assert.doesNotMatch(reportOpening, /rankLabel/, "PD-06 : rang hors résumé");
assert.doesNotMatch(reportOpening, /publishedAt/, "PD-06 : publié le hors résumé");
assert.doesNotMatch(reportOpening, /<Metric/, "PD-06 : métriques hors résumé");
assert.match(reportCard, /Visionner le bulletin/, "PD-06 : PDF uniquement carte ouverte");
