...
function main() {
  ...
  assert.match(tabs, /SomafrikBottomTabBar/);
  assert.match(tabs, /tabBar=\{/);
  assert.match(tabs, /shortBottomTabLabel/);
  assert.doesNotMatch(tabs, /CompactTabButton/);
  assert.doesNotMatch(tabs, /TAB_ITEM_INNER_ALIGN/);
  assert.doesNotMatch(tabs, /tabVerticalUiKit/);
  assert.doesNotMatch(tabs, /translateY/);
  assert.doesNotMatch(tabs, /resolveTabBarBottomInset/);
  const ownerBar = read("Mobile/src/navigation/SomafrikBottomTabBar.tsx");
  assert.match(ownerBar, /SOMAFRIK_TAB_SPACE_BELOW_LABEL_DP/);
  assert.match(ownerBar, /useFloatingTabBarLayout/);
  assert.match(ownerBar, /NAVIGATION_TEST_IDS\.tabBar|mobile-tab-bar/);
  assert.match(ownerBar, /tabBarButtonTestID/);
  assert.doesNotMatch(ownerBar, /views\/BottomTabBar/);
  assert.doesNotMatch(ownerBar, /tabVerticalUiKit/);
  assert.doesNotMatch(ownerBar, /CompactTabButton/);
  assert.doesNotMatch(ownerBar, /@somafrik\/help-catalog/);
  assert.doesNotMatch(ownerBar, /HelpHost|HELP-V1C|help-v1c/);
